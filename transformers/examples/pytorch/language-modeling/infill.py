    ...
    if args.modality in ['synth', 'pos']:
        ...
        if args.eval_task_ == 'infill':
            ...
            if args.eval_task_ == 'control_attribute_compose':
                ...
                ##################### NOTE TO AMIR: Changed classifier path to be an argument #######
                # also changed cuda to cpu... 
                model_control = Classifier_Tree.from_pretrained(
                    args.classifier_path).cuda()

                # print(model_control)

                import benepar
                ###################### NOTE TO AMIR: Commenting out #################################
                ...
                ###############
                control_label_lst = []
                ###################### NOTE TO AMIR: Copied tree and changed path ###################
                filepath = 'diffusion_lm/improved-diffusion/control_gen/target_tree.json'
                filepath = 'target_tree.json'
                #####################################################################################
                with open(filepath,
                          'r') as controlf:
                    for line in controlf:
    ...
    if True:
        for (encoded_seq, control_helper) in zip(encoded_partial_seq, partial_seq) :
            ...
            while len(all_images) * args.batch_size < args.num_samples:
                ...
                ################################## NOTE TO AMIR: Changeing GPU to CPU #######################
                ...
                print(f'Amir says that my evaluation task is {args.eval_task_}')
            ...
            ############################################
            # Commenting out so that we can decode
            # multiple timestamps per sample
